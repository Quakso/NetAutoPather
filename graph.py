...
class Graph:
    ...
    # 将时延map放入对应位置
    def putDelayMap(self, delayMap: dict):
        for keySrc in delayMap.keys():
            dstValue = delayMap.get(keySrc)
            keySrc=keySrc.split('/')
            srcPoint=Point(keySrc[0],keySrc[1])
            for keyDst in dstValue.keys():
                tmp=dstValue.get(keyDst)
                if len(tmp)==0:
                    tmp=1
                else:
                    tmp=int(tmp)
                delayWeight=tmp
                keyDst=keyDst.split('/')
                dstPoint=Point(keyDst[0],keyDst[1])
                self.weightMatrix[srcPoint][dstPoint]=delayWeight
        # 放入时延权重后打印Map
        self.printMap()
